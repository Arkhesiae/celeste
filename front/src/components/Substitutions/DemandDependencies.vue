    ...
    <ConfirmationDialog v-model="showConfirmationDialog" :title="dialogTitle" :text="dialogText">
        <template #actions>
            ...
            <template v-if="dialogOption === 'accept'">
                <v-btn v-if="userHasShift && canSwitch"  variant="tonal" rounded="xl"
                    :loading="loading.accept" @click="handleConfirmSwitch">
                    Permuter
                </v-btn>
                <v-btn color="primary" variant="flat" rounded="xl" :loading="loading.accept"
