<template>
  <v-card color="surfaceContainerHigh">
      ...
      <template v-else>
        ...
        <template v-else>
          ...
          <v-chip-group v-model="selectedVariant" color="background" variant="flat" class="mb-4">
            ...
            <v-chip v-for="(variant, index) in day?.variants" :key="index" :value="index" color="onBackground"
          ...
          <div class="d-flex justify-space-between align-center">
            <div>
              <div class="text-caption text-medium-emphasis">Début</div>
              <div class="text-h6">{{ selectedVariant === 'default' ? (day?.startTime || '--:--') :
                (day?.variants[selectedVariant]?.startTime || '--:--') }}</div>
            </div>
            <div>
              <div class="text-caption text-medium-emphasis">Fin</div>
              <div class="text-h6">{{ selectedVariant === 'default' ? (day?.endTime || '--:--') :
                (day?.variants[selectedVariant]?.endTime || '--:--') }}</div>
            </div>
          </div>
        </template>
      </template>
    ...
    <v-card-actions class="pa-6">
      ...
      <v-btn size="small" slim v-if="authStore.isAdmin && day?.type !== 'rest'" prepend-icon="mdi-pencil" variant="text" class="mr-2"
        @click="$emit('onEdit')">Modifier</v-btn>
    </v-card-actions>
  </v-card>
