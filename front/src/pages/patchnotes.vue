    ...
    <v-row>
      ...
      <v-col cols="12" md="4" lg="3">
        ...
        <template v-if="!smAndDown">
          <EntitySelector
:items="filteredVersions" item-key="id" item-title="version" :item-subtitle="'releaseDate'"
            :item-status="'status'" :model-value="selectedVersion" title="Versions" max-height="70vh"
            ...
            <template #statusChip="{ item }">
              ...
              <v-chip
v-else-if="item.status === 'current'" size="x-small" color="onBackground" variant="flat"
                rounded="lg">
                Actuelle
              </v-chip>
        ...
        <template v-else>
          <VersionSelector
:model-value="selectedVersion" :width="400"
            ...
            <template #statusChip>
              <v-chip
v-if="selectedVersion?.status === 'future'" size="x-small" color="warning" variant="tonal"
                rounded="lg">
                Future
              </v-chip>
              <v-chip
v-else-if="selectedVersion?.status === 'current'" size="x-small" color="onBackground"
            ...
            <template #dialog>
              <EntitySelector
:items="filteredVersions" item-key="id" item-title="version"
                :item-subtitle="'releaseDate'" :item-status="'status'" :model-value="selectedVersion"
                title="Sélectionner une version" max-height="40vh" @update:model-value="selectVersionFromDialog">
                <template #statusChip="{ item }">
                  <v-chip v-if="item.status === 'future'" size="x-small" color="warning" variant="tonal" rounded="lg">
                    Future
                  </v-chip>
...
v-else-if="item.status === 'current'" size="x-small" color="onBackground" variant="flat"
      ...
      <v-col cols="12" md="8" lg="9">
        <v-card
v-if="selectedVersion" :class="!smAndDown ? 'pl-16' : 'pl-2 pr-2'" rounded="xl" elevation="0"
          class="smooth-shadow pa-2 pt-16" color="transparent">
          ...
          <div v-if="selectedVersion.improvements?.length > 0" class="mb-6">
            ...
            <v-list class="bg-transparent">
              <v-list-item
v-for="(improvement, index) in selectedVersion.improvements" :key="index"
          ...
          <div v-if="selectedVersion.bugFixes?.length > 0" class="mb-6">
            ...
            <v-list class="bg-transparent">
              <v-list-item
v-for="(bugFix, index) in selectedVersion.bugFixes" :key="index"
          ...
          <div v-if="selectedVersion.technicalNotes?.length > 0" class="mb-6">
            ...
            <v-list class="bg-transparent">
              <v-list-item
v-for="(note, index) in selectedVersion.technicalNotes" :key="index"
