            ...
            <v-col cols="12" md="6" style="transition: all 0.3s ease-in-out;">
              <v-window v-model="step" class="mt-8" style="transition: all 0.3s ease-in-out;">
                ...
                <v-window-item :value="1">
                  ...
                  <v-form>
                    <v-text-field
v-model="form.login" label="Email ou login de l'ancien compte" required rounded="xl"
                      variant="solo-filled" flat :rules="[v => !!v || 'L\'email est requis']" class="mb-4" />
                    <v-select
v-model="form.center" label="Centre" required rounded="xl" variant="solo-filled" flat
                      :rules="[v => !!v || 'Le centre est requis']" class="mb-4" :items="centers" item-title="name"
                      item-value="OACI" />
                    <v-alert
v-if="step1Error" color="error" rounded="xl" variant="tonal"
...
color="permutation" class="text-body-large" :value="false"
                  ...
                  <div class="mt-4" style="transition: all 0.3s ease-in-out;">
                    <v-slide-y-transition>
                      <v-text-field
v-show="!useOldEmail" v-model="newEmail" label="Nouvelle adresse email" type="email"
                        rounded="xl" variant="solo-filled" flat
                        :rules="[v => !!v || 'L\'email est requis', v => /.+@.+\..+/.test(v) || 'Email invalide']"
                        class="mb-2" />
                    </v-slide-y-transition>
                  </div>
                  <v-alert
v-if="step2Error" type="error" class="mb-2" color="error" variant="tonal" rounded="xl"
...
v-if="!otpVerified" :email="useOldEmail ? oldAccount.email : newEmail"
                ...
                <v-window-item :value="4">
                  ...
                  <v-form>
                    <v-text-field
v-model="newPassword" rounded="xl" variant="solo-filled" flat
                      autocomplete="new-password" label="Nouveau mot de passe" type="password" :rules="passwordRules"
                      class="mb-2" />
                    <v-text-field
v-model="confirmPassword" rounded="xl" variant="solo-filled" flat
                      autocomplete="new-password" label="Confirmer le mot de passe" type="password"
                      :rules="confirmPasswordRules" class="mb-2" />
                    <v-alert
v-if="step4Error" type="error" class="mb-2" color="error" variant="tonal" rounded="xl"
                      icon="mdi-alert-outline">
                      {{ step4Error }}
                    </v-alert>
                  </v-form>
          ...
          <v-row>
            <v-col cols="12" md="6" offset-md="6">
              <v-card-actions class="mt-4">
                <v-btn v-if="step > 1" variant="text" color="onSurface" @click="goBack">
                  Retour
                </v-btn>
                <v-spacer />
                <v-btn
v-if="step < 4" variant="flat" color="onBackground" rounded="lg" append-icon="mdi-arrow-right"
                  :disabled="!canGoNext" @click="goNext">
                  Continuer
                </v-btn>
                <v-btn
v-if="step === 4" variant="flat" color="onBackground" rounded="lg" append-icon="mdi-arrow-right"
                  :disabled="!canGoNext" @click="handleRecovery">
                  Récupérer
                </v-btn>
                <v-btn
v-if="step === 5" variant="flat" color="onBackground" rounded="lg" append-icon="mdi-arrow-right"
