...
watch(() => props.existingDay, (newValue) => {
  ...
  <v-dialog v-model="localDialogVisible" max-width="500">
    <v-card rounded="xl" elevation="0" color="surfaceContainer">
      <v-card-item class="pa-6  mb-4">
        ...
        <v-card-subtitle class="text-medium-emphasis">
          {{ isEditMode ? 'Modifiez les horaires et les points de cette vacation' : 'Configurez les horaires et les points pour cette vacation' }}
        </v-card-subtitle>
        <template #append>
    ...
    <v-card-title class="text-subtitle-1 font-weight-medium pa-0 mb-4">Variantes</v-card-title> 
        ...
        <div v-for="(variant, index) in newDay.variants" :key="index" class="mb-1">
          <v-card class="d-flex align-center justify-space-between" flat color="transparent" height="48" :rounded="smAndDown ? 'xl' : 'lg'">
