  ...
  >
    <v-sheet class="pa-4">
      <v-card-item class="py-1 px-6 mb-2">
       ...
       <v-card-title class="d-flex justify-space-between align-center">Permutations disponibles</v-card-title>
       ...
       <template #prepend v-else>
        ...
        <DemandCard
          v-for="substitution in availableSwitches"
          :key="substitution.id"
          :demand="substitution"
        ></DemandCard>
...
const handleAccept = async (switch_) => {
  try {
    await substitutionStore.acceptDemand(switch_._id);
    emit('update:modelValue', false);
  } catch (error) {
    console.error('Erreur lors de l\'acceptation de la permutation:', error);
  }
};
</script>
